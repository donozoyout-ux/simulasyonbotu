...
function MarketMemoryView({symbols,health,backfill}:{symbols:string[];health?:MarketMemoryHealth;backfill?:BackfillStatus}){
  ...
  const [timeframe,setMemoryTimeframe]=useState("15m"),[range,setRange]=useState("1M"),[anchorAt,setAnchorAt]=useState("");
  ...
  const levelSnapshot=chartSnapshots.at(-1)||latestSnapshot;
  const levels=levelSnapshot&&(levelSnapshot.support!=null||levelSnapshot.resistance!=null)?{support:levelSnapshot.support,resistance:levelSnapshot.resistance}:undefined;
  const source=Array.from(new Set(candles.map(item=>item.source).filter(Boolean))).join(" / ")||"DB";
  return <div className="memory-layout"><article className="panel memory-price-panel">
    <div className="chart-head"><div><b>{symbol} • {timeframe.toUpperCase()}</b><span>Gerçek historical OHLC • DB-first</span></div><select value={symbol} onChange={event=>{setAnchorAt("");setTimeline([]);setArchive([]);setSymbol(event.target.value)}}>{Array.from(new Set([symbol,...symbols])).map(item=><option key={item}>{item}</option>)}</select></div>
    <div className="memory-header-stats"><div><span>Latest Price</span><b>{latestCandle?money(latestCandle.close):latestSnapshot?.price!=null?money(latestSnapshot.price):"—"}</b></div><div><span>Snapshots</span><b>{timeline.length}</b></div><div><span>Candles loaded</span><b>{candles.length}</b></div><div><span>Last candle</span><b>{lastTime?fmtDate(lastTime):"—"}</b></div><div><span>Data source</span><b>{source}</b></div></div>
    <div className="memory-controls"><div className="timeframes">{["5m","15m","1h","1d"].map(value=><button key={value} className={timeframe===value?"active":""} onClick={()=>setMemoryTimeframe(value)}>{value.toUpperCase()}</button>)}</div><div className="timeframes">{["1D","5D","1M","3M","ALL"].map(value=><button key={value} className={range===value?"active":""} onClick={()=>setRange(value)}>{value==="1D"?"Son 1 Gün":value==="5D"?"Son 5 Gün":value==="1M"?"Son 1 Ay":value==="3M"?"Son 3 Ay":"Tümü"}</button>)}</div>{anchorAt&&<button className="pause-control" onClick={()=>setAnchorAt("")}>Güncele dön</button>}</div>
    {candleLoading?<div className="chart-placeholder"><RefreshCw className="spin"/><b>Historical candles yükleniyor...</b></div>:candleError?<div className="chart-placeholder"><CandlestickChart/><b>Candle verisi alınamadı</b><span>{candleError}</span></div>:candles.length?<PriceChart candles={candles} levels={levels} news={chartNews} snapshots={chartSnapshots}/>:<div className="chart-placeholder"><CandlestickChart/><b>Bu sembol/timeframe için historical candle bulunamadı.</b><span>Grafik yalnızca Candle tablosundaki gerçek OHLC verisini gösterir.</span></div>}
    <div className="memory-timeline">{timeline.length?timeline.map(item=><button key={item.id??item.timestamp} onClick={()=>setAnchorAt(item.timestamp)}><time>{fmtDate(item.timestamp)}</time><b>{item.price!=null?money(item.price):"—"}</b><span>{item.trend||"—"} • {item.market_structure||"—"} • skor {item.technical_score??"—"} • {item.analysis_mode||"—"}</span></button>):<span>Snapshot henüz yok; historical candle grafiği bağımsız olarak görüntülenir.</span>}</div>
  </article><aside className="panel"><PanelTitle title="Backfill & Arşiv" sub="Cursor tabanlı, tekrar başlatılabilir"/><div className="health-row">{backfill?.tasks.map(item=><span key={item.task}><b>{item.task}</b> {item.status} • cursor {item.cursor} • {item.processed_items} işlendi</span>)}</div><h3>{symbol} haber zaman çizgisi</h3><div className="chart-news-list">{archive.slice(-10).reverse().map(item=><a key={item.id} href={item.url} target="_blank" rel="noreferrer"><b>{item.source} {item.overnight_news?"• GECE":""}</b> {item.title}</a>)}</div><h3>Event study</h3><div className="health-row">{study.map(item=><span key={item.category}><b>{item.category}</b> n={item.news_count} • 1G {item.avg_return_1d?.toFixed(2)??"—"}% • pozitif %{item.positive_rate?.toFixed(0)??"—"}</span>)}</div></aside></div>
}
