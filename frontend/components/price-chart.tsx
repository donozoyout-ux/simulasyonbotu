...
export function PriceChart({candles,levels,swings=[],entries=[],exits=[],news=[]}:{candles:Candle[];levels?:{support:number;resistance:number;supportLow?:number;supportHigh?:number;resistanceLow?:number;resistanceHigh?:number;entry?:number;stop?:number;target?:number};swings?:Array<{timestamp:string;price:number;type:string}>;entries?:TradeMarker[];exits?:TradeMarker[];news?:NewsItem[]}){
  ...
  useEffect(()=>{
    ...
    chart.subscribeClick(param=>{const id=String(param.hoveredObjectId||"");if(id.startsWith("news:")){const key=id.slice(5);setSelectedNews(news.find(item=>String(item.id??item.source_id)===key))}});
